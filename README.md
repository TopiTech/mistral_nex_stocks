...
- `npm run build` - 型チェック、compile、verify-generated、lint、Prettier 検証 / Typecheck, compile, verify-generated, lint, and Prettier checks.
...
| 真偽フラグ共通（`MNS_ALLOW_REMOTE_API` / `MNS_PROXY_FIX` / `MNS_COOKIE_SECURE` / `MNS_STRICT_SEC_FETCH_SITE` / `CSP_ENFORCE` 等）| `0` | 真値は `1` / `true` / `yes` / `on`（大文字小文字不問・前後空白可）。それ以外は偽かデフォルト。`on` も真として扱われます / Truthy values are `1` / `true` / `yes` / `on` (case-insensitive). |
...
| `MNS_STRICT_SEC_FETCH_SITE`                 | `0`                      | `1` で Sec-Fetch-Site:none/missing時に Origin必須化(ループバックの非ブラウザ POSTを拒否)。デフォルト 0はローカル個人利用の互換性を優先/ When 1, mutating POSTs with Sec-Fetch-Site:none/missing without trusted Origin are blocked (non-browser loopback guard). Default 0 preserves local-first compatibility. |
| `CSP_ENFORCE`                               | `1`                      | CSP を強制するか Report-Only にするか / Controls whether CSP is enforced or report-only.                                                                                                                                                                                                                        |
| `MNS_COOKIE_SECURE`                         | `0`                      | セッションクッキーの Secure 属性。`1` のみHTTPローカル環境でクッキーが送信されずセッションが機能しなくなるため、HTTPで使う場合は設定しないこと（HTTPS環境専用） / Forces Secure cookies. Do not set on plain-HTTP localhost (cookies stop working); HTTPS only. |
...
| `MNS_MISTRAL_SDK_RETRIES`                   | `2`                      | SDK内リトライ回数の目安（各API試行のタイムアウト＋バックオフを含むRetryConfig時間予算へ変換）/ Approximate SDK retry attempts, translated to the SDK v2 RetryConfig time budget including per-attempt timeouts and backoff. |
...
| `MNS_LANGSEARCH_TOTAL_TIMEOUT`              | `30.0`                   | LangSearch 1回のHTTP処理（リトライ・待機を含む）の最大時間（秒）/ Maximum wall-clock duration for one LangSearch operation including retries and backoff (seconds).                                                                                                                                             |
...
| `MNS_PROXY_FIX_X_FOR`                       | `1`                      | ProxyFix が信頼する `X-Forwarded-For` ホップ数。`MNS_ALLOW_REMOTE_API=1` 時は `>=1` 必須(0以下は起動時 FATAL)/ Number of trusted proxy hops for ProxyFix. Must be >=1 when remote mode is enabled (0 triggers fatal startup error).                                                                             |
| `MNS_DATA_DIR` / `MNS_APP_DATA_DIR`         | _(platform default)_     | ランタイムデータディレクトリの上書き / Override runtime data directory (config, user_stocks, chat_history).                                                                                                                                                                                                     |
| `NATIVE_HOST_MAX_MESSAGE_BYTES`             | `1048576`                | native host のメッセージ上限 / Native host message size limit.                                                                                                                                                                                                                                                  |
| `NATIVE_HOST_MAX_DRAIN_BYTES`               | `2097152`                | native host のドレイン上限 / Native host drain buffer limit.                                                                                                                                                                                                                                                    |
| `NATIVE_HOST_RATE_LIMIT_MAX`                | `10`                     | native host のレート制限回数 / Native host rate limit count.                                                                                                                                                                                                                                                    |
| `NATIVE_HOST_RATE_LIMIT_WINDOW`             | `1`                      | native host のレート制限ウィンドウ(秒)/ Native host rate limit window (sec).                                                                                                                                                                                                                                    |
| `NATIVE_HOST_TOKEN_ACTION_MAX`              | `3`                      | native host のトークン取得系アクション上限 / Native host token-action limit.                                                                                                                                                                                                                                    |
| `NATIVE_HOST_TOKEN_ACTION_WINDOW`           | `30`                     | native host のトークン取得ウィンドウ(秒)/ Native host token-action window (sec).                                                                                                                                                                                                                                |
...
- 開発時は `uv run --locked python app.py` で起動できます。`run_app.sh` も `uv` が利用できる場合は常に同じ locked 環境を通して起動します。`uv` がない場合は `uv sync --locked` を先に実行してください。 / For development, run `uv run --locked python app.py`. `run_app.sh` also always starts through the same locked environment when `uv` is available. If `uv` is unavailable, run `uv sync --locked` first.
