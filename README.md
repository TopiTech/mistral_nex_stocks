...
- 配布や運用で WSGI (Gunicorn等) を使う場合は [wsgi.py](wsgi.py) と [gunicorn.conf.py](gunicorn.conf.py) を参照してください。本アプリはインメモリ状態依存のため、**単一ワーカープロセス（`workers=1` / `WEB_CONCURRENCY=1`）での起動が必須**です。複数ワーカー起動は禁止されており自動保護されています。 / For WSGI deployment, see [wsgi.py](wsgi.py) and [gunicorn.conf.py](gunicorn.conf.py). Due to in-memory state dependencies, **starting with a single worker process (`workers=1` / `WEB_CONCURRENCY=1`) is strictly required**. Multi-worker mode is unsupported and guarded against.
