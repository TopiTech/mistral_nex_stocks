...
| `MNS_STRICT_SEC_FETCH_SITE`                 | `0`                      | `1` で Sec-Fetch-Site:none/missing時に Origin必須化(ループバックの非ブラウザ POSTを拒否)。デフォルト 0はローカル個人利用の互換性を優先/ When 1, mutating POSTs with Sec-Fetch-Site:none/missing without trusted Origin are blocked (non-browser loopback guard). Default 0 preserves local-first compatibility.                 |                      | ProxyFix を有効化するフラグ / Enables Werkzeug ProxyFix.                                                                                                                                                               |
