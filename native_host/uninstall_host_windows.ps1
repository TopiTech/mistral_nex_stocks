...
function Open-RegistryRoot {
  ...
  if ($InstallScope -eq 'LocalMachine') {
...
function Get-SubKeys {
  ...
  if ($BrowserName -in @('Chrome','Both')) {
    [void]$keys.Add('SOFTWARE\Google\Chrome\NativeMessagingHosts\com.mistral_nex_stocks.host')
  }
  if ($BrowserName -in @('Edge','Both')) {
    [void]$keys.Add('SOFTWARE\Microsoft\Edge\NativeMessagingHosts\com.mistral_nex_stocks.host')
  }
  return $keys
}
...
foreach ($subKey in (Get-SubKeys -BrowserName $Browser)) {
  if ($PSCmdlet.ShouldProcess("$Scope\$subKey", "Remove NativeMessagingHosts registry key")) {
    try {
