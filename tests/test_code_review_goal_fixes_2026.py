...
def test_leader_lock_release_preserves_lock_file(tmp_path):
    ...
    mock_f = MagicMock()
    with patch('bg.leader_election._get_leader_lock_file', return_value=mock_f), \
...
def test_stock_service_transient_failure_does_not_poison_negative_cache():
    with patch('services.stock_service.safe_get_ticker', return_value=None), \
...
def test_embeddings_service_bounds_check():
    ...
    data_mock = [MagicMock(embedding=[0.1, 0.2]), MagicMock(embedding=[0.3, 0.4]), MagicMock(embedding=[0.5, 0.6])]
    client_mock.embeddings.create.return_value = MagicMock(data=data_mock, usage=type('Usage', (), {'total_tokens': 10})())

    texts = ['hello', 'world']
    with patch('services.embeddings_service._get_client', return_value=client_mock), \
...
def test_routes_pages_safe_template_context_error_boundary():
    with patch('utils.validators.DefaultSymbolsSchema.model_validate', side_effect=ValueError('Schema error')), \
         patch('utils.validators.AppConfigSchema.model_validate', side_effect=ValueError('Schema error')):
...
def test_info_fetch_inflight_cleanup_on_exception():
    ...
    with patch.object(app_state.execution.data_executor, 'submit', side_effect=RuntimeError('Thread pool error')):
        with app.test_request_context():
...
def test_api_chat_standardized_forbidden_response():
    ...
    with app.test_request_context('/api/chat', method='POST', json={'message': 'hello'}):
        with patch('routes.api_analysis.require_trusted_or_admin', return_value=(False, 'forbidden')):
            resp = api_chat()
