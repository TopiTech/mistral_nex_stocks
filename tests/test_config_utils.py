...
class ConfigUtilsTestCase(unittest.TestCase):
    ...
    def test_save_config_creates_backup_and_sets_permissions_on_unix(self):
        ...
        with (
            ...
            patch.object(config_store.os, "chmod") as chmod_mock,
        ):
    ...
    def test_save_api_credentials_stores_encoded_blob(self):
        with patch.object(
            crypto_utils, "_encode_secret", return_value={"scheme": "test", "value": "abc123"}
        ):
    ...
    def test_master_key_generation_is_shared_by_concurrent_callers(self):
        with (
            patch.object(
                crypto_utils,
                "_encode_secret",
                return_value={"scheme": "fernet", "value": "testkey"},
            ),
            patch.object(crypto_utils, "_decode_secret", return_value="testkey"),
        ):
            with ThreadPoolExecutor(max_workers=2) as executor:
                keys = list(
                    executor.map(lambda _: config_store.get_or_create_master_key(), range(2))
                )
    ...
    def test_clear_api_credentials_removes_keyring_entries(self):
        ...
        with (
            patch.object(crypto_utils, "KEYRING_AVAILABLE", True),
            patch.object(crypto_utils.keyring, "delete_password") as delete_password_mock,
        ):
    ...
    def test_save_api_credentials_rejects_plaintext_without_keyring(self):
        ...
        with (
            patch.object(crypto_utils, "KEYRING_AVAILABLE", False),
